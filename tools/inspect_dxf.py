...
INSUNITS = {
    ...
    5: "centímetros", 6: "metros", 14: "decímetros",
}
...
def load(path: Path):
    if path.suffix.lower() == ".dwg":
        ...
        print(f"[i] Convirtiendo DWG → DXF vía ODA File Converter…")
...
def main() -> None:
    ...
    with_width = sum(
        1 for p in polys
        if p.dxf.hasattr("const_width") and p.dxf.const_width > 0
    )
    print(f"\n--- Polilíneas: {len(polys)} (con ancho constante: {with_width}) ---")
    if polys and with_width == 0:
        print("[!] Ninguna polilínea tiene ancho: los muros probablemente son"
    ...
    if ext_min and ext_max:
        ...
        print("(una casa debería medir ~10-30 si está en metros,"
              " ~10000-30000 si está en mm)")
